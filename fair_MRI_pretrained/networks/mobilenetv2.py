...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, sample_size=224, in_channel=1, width_mult=1.0):
        ...
        self.features = [conv_bn(in_channel, input_channel, (2, 2, 2))]
    ...
    def forward(self, x, get_inter=False):
        x = self.features(x)
        pooled_x = F.avg_pool3d(x, x.data.size()[-3:])
        h = pooled_x.view(pooled_x.size(0), -1)
        outs = self.classifier(h)
        outs = outs.squeeze()
