...
class GenericTrainer:
    def __init__(self, args, log_dir=None, log_name=None, save_dir=None, **kwargs):
        ...
        if self.binary:
            if self.method == 'WCE' or self.method == 'RW':
    ...
    def _get_optim_n_scheduler(self, args, model):
        if isinstance(model, list):
            ...
            params = [
                { 'params': self.parameters['base_parameters'], 'lr': args.lr }, 
                { 'params': self.parameters['new_parameters'], 'lr': args.lr }
                ]
    ...
    def evaluate(self, model, loader, criterion, cuda=True, logger = None):
        ...
        lbc = True if self.method == 'LBC' else False
        ...
        with torch.no_grad():
            ...
            for j, eval_data in enumerate(loader):
                ...
                outputs_male = outputs[torch.where(gender==0)[0]] #
                outputs_female = outputs[torch.where(gender==1)[0]] #
                labels_male = labels[torch.where(gender==0)[0]] #
                labels_female = labels[torch.where(gender==1)[0]] #
                
                loss = criterion(outputs, labels)
                if self.method == 'WCE' or self.method == 'RW':
                    loss = loss * weight.cuda()
                    loss = torch.sum(loss)
                elif self.method =='LBC':
                    loss = torch.sum(loss)
            ...
            if num_classes ==3: #with-mci
                micro_f1 = F1Score(num_classes=3, average="micro")(torch.cat(yhats), torch.cat(ys)).item()
                macro_f1 = F1Score(num_classes=3, average="macro")(torch.cat(yhats), torch.cat(ys)).item()

            if self.mode == 'eval':        
                logger = self.logger if logger is None else logger
                # test_acc = eval_acc.item()
                ...
                #return eval_loss.item(),  eval_acc.item(), bal_acc, (male_acc_fair, female_acc_fair), (roc_auc, fpr, tpr), (roc_auc_male, fpr_male, tpr_male), (roc_auc_female, fpr_female, tpr_female)
